...
A session store, such as Redis, allows the web server to scale up and down with instances and still have access to the same session store as the other instances.
...
Strictly typed languages like Elm is functional and was discarded due to the learning curve of such a language did not suite the time budget for the project.
...
\todo[inline]{Den här meningen får det att låta lite som att TypeScript körs i webbläsaren? "thus giving the option to use standard JavaScript if needed", allt är ju JavaScript i slutändan}
